"""
Module Name: spaces.py
Author: Alice Bizeul, taken and adapted from https://github.com/brendel-group/cl-ica
Description: Utility functions for spaces.
"""
...
class NRealSpace(Space):
    ...
    def truncated_laplace(self,loc, scale, lower, upper, size,device="cpu"):
        if not torch.is_tensor(scale):
            scale = torch.ones(self.n) * scale
        if not torch.is_tensor(lower):
            lower = torch.ones(self.n) * lower
        if not torch.is_tensor(upper):
            upper = torch.ones(self.n) * upper
        cdf_lower = 0.5 * torch.exp((lower - loc) / scale)
        cdf_upper = 1 - 0.5 * torch.exp((loc - upper) / scale)
        u = torch.rand(size) * (cdf_upper - cdf_lower) + cdf_lower
        samples = torch.where(
            u < 0.5,
...
class NSphereSpace(Space):
    ...
    def truncated_laplace(self,loc, scale, lower, upper, size, device="cpu"):
        ...
        cdf_lower = 0.5 * torch.exp((lower - loc) / scale)
        cdf_upper = 1 - 0.5 * torch.exp((loc - upper) / scale)
        u = torch.rand((size, self.n),device=device) * (cdf_upper - cdf_lower) + cdf_lower
        samples = torch.where(
            u < 0.5,
            loc + scale * torch.log(2 * u),
            loc - scale * torch.log(2 * (1 - u))
