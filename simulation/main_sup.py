...
def parse_args():
    ...
    parser.add_argument(
        "--save_folder", type=str, default="./sup", help="Dimensionality of the latents and observed variables. "
        "Potential values to start with: 5, 10."
    )
...
def main():
    ...
    tag=f"seed_{args.seed}_nsamp_{args.nr_indices}_n_{args.n}_nclusters_{args.nr_clusters}_epochs_{args.n_epochs}_mp_{args.m_p}_mparam_{args.m_param}_cp_{args.c_p}_cparam_{args.c_param}_batch_{args.batch_size}_{args.sphere_norm}"
    ...
    with open(os.path.join(args.save_folder,f'performance_linear_{tag}.csv'), 'w', newline='') as csvfile:
        ...
        writer.writerow(['zv','step','seed','nb indices', 'batch size', 'nb clusters', 'n', 'm', 'm param', 'c', 'c param', 'z norm', 'w bias','final loss', 'epochs', 'z2z r2'])
        for _, (step,v_1) in enumerate(linear_disentanglement_scores):
            writer.writerow([True,step,args.seed,args.nr_indices,args.batch_size,args.nr_clusters,args.n,args.m_p,args.m_param,args.c_p,args.c_param,args.sphere_norm,args.W_bias_used,losses[-1][1].item(),args.n_epochs,v_1])  
